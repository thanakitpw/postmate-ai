...
CREATE TABLE monthly_plan_configs (
  ...
  active_days           int[] NOT NULL,      -- [1,3,5,6] = จ,พ,ศ,ส (0=อา,1=จ,...,6=ส)
  default_posts_per_day int DEFAULT 1,
  day_overrides         jsonb DEFAULT '{}',  -- {"6": 2} = เสาร์ 2 โพสต์
...
CREATE TABLE posts (
  ...
  article_url       text,                     -- สำหรับ article_share type
...
CREATE TABLE post_results (
  ...
  error_message    text,
  platform_post_id text,     -- ID ของโพสต์จริงบน platform
  screenshot_url   text,     -- screenshot เมื่อล้มเหลว (Supabase Storage)
...
-- Service role สำหรับ VPS Playwright insert results
CREATE POLICY "Service role can insert results"
  ON post_results FOR INSERT
  WITH CHECK (true);  -- จำกัดด้วย service_role key
